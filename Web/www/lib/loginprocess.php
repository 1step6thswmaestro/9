...
if ($type=="logout" || $type=="exit") {
	...
	if($type=="logout") {
		...
		if(false !== mysql_query($sql,get_db_conn()) && $_ShopInfo->getTempkey() !=""){
			// 로그아웃시 장바구니 비우기
			/*$delBasket = "DELETE FROM tblbasket WHERE tempkey = '".$_ShopInfo->getTempkey()."' ";
			@mysql_query($delBasket,get_db_conn());
			$delBasket2 = "DELETE FROM tblbasket2 WHERE tempkey = '".$_ShopInfo->getTempkey()."' ";
			@mysql_query($delBasket2,get_db_conn());
			$delBasket3 = "DELETE FROM tblbasket3 WHERE tempkey = '".$_ShopInfo->getTempkey()."' ";
			@mysql_query($delBasket3,get_db_conn());
			$delBasket4 = "DELETE FROM tblbasket4 WHERE tempkey = '".$_ShopInfo->getTempkey()."' ";
...
$sql = "SELECT * FROM tblmember WHERE member_out ='N' AND id='".$id."' ";
if($ssllogintype=="ssl") {
