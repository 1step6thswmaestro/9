...
if($exec == "admin") {
	if($member[admin]=="SU") {
		header("Location:board.php?pagetype=list&board=$board&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage&boardsep=$boardsep");
		exit;
	}
	...
	if ($exec == "delete") {
		if ($member[admin]=="SU") {
			header("Location:board.php?pagetype=delete&board=$board&num=$num&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage&boardsep=$boardsep");
			exit;
		}
		$html_url = "board.php?pagetype=delete";
	} else if ($exec == "modify") {
		if ( ( $member[admin]=="SU" ) OR ( $member[id]!="" AND $boardUserid == $member[id] ) ) {
			header("Location:board.php?pagetype=write&exec=$exec&board=$board&num=$num&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage&boardsep=$boardsep");
			exit;
		}
		...
		if ($isSecret || $member[admin]=="SU") {
			header("Location:board.php?pagetype=view&board=$board&num=$num&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage&boardsep=$boardsep");
			exit;
		}
		$html_url = "board.php?pagetype=view&view=1&board=$board&num=$num&s_check=$s_check&search=$search&block=$block&gotopage=$gotopage&boardsep=$boardsep";
