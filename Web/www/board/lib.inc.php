	...
	if ($setup[use_lock]!="N") {
		...
		if ($view=="1") {
			if ($isSecret || $member[admin]=="SU") {
				...
				header("Location:board.php?pagetype=view&num=$num&board=$board&block=$block&gotopage=$gotopage&search=$search&s_check=$s_check&boardsep=$boardsep");
				exit;
			} else {

				header("Location:board.php?pagetype=passwd_confirm&exec=secret&top_num=$rowset[num]&num=$num&board=$board&block=$block&gotopage=$gotopage&search=$search&s_check=$s_check&boardsep=$boardsep&error=$error");
				exit;
			}
		} else {
			if (!$isSecret && $member[admin]!="SU") {
				header("Location:board.php?pagetype=passwd_confirm&board=$board&exec=secret&top_num=$rowset[num]&num=$num&block=$block&gotopage=$gotopage&search=$search&s_check=$s_check&boardsep=$boardsep&error=$error");
				exit;
			}
		}
	} else {
