...
function comment_delete(num,c_num,frametype) {
	if(frametype=="Y") {
		...
		document.location.href="board.php?pagetype=delete_comment&board=<?=$board?>&num="+num+"&c_num="+c_num+"&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>&boardsep=<?=$boardsep?>";
	}
}
//-->
</SCRIPT>