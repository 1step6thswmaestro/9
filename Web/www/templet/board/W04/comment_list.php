			...
			<TABLE cellSpacing="0" cellPadding="0" width="100%">
				...
				<TR>
					...
					<td align="right">
						...
						<? if( $setup["onlyCmt"] == "N" OR strlen($_ShopInfo->id) > 0 ){ ?>&nbsp;<IMG src="<?=$imgdir?>/board_del.gif" border="0" align="absmiddle" style="CURSOR:pointer;" onclick="location='board.php?pagetype=delete_comment&board=<?=$board?>&num=<?=$num?>&c_num=<?=$c_num?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>&boardsep=<?=$boardsep?>'"></font><? } ?>
					</TD>
				</TR>
