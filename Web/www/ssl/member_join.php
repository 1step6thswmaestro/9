...
if(strlen(trim($id))==0) {
	...
	echo "<html><head><title></title></head><body onload=\"alert('집 우편번호 입력이 잘못되었습니다.');history.go(-1)\"></body></html>";exit;
} else if(strlen($office_post1) > 0 && !preg_match('/[0-9]{5}/',$office_post1)){
	echo "<html><head><title></title></head><body onload=\"alert('회사 우편번호 입력이 잘못되었습니다.');history.go(-1)\"></body></html>";exit;
}else {
	$procdata=array();
