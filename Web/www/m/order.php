...
if($basketItems['sumprice']<$_data->bank_miniprice) {
	echo "<script>alert('주문 가능한 최소 금액은 ".number_format($_data->bank_miniprice)."원 입니다.');location.href='./basket.php';</script>";
	exit;
...
<!-- <input type=hidden name=paymethod> --><!-- 결제를현재페이지에서하기위해 주석처리 -->
<!-- <input type=hidden name=pay_data1> --><!-- 결제를현재창페이지서하기위해 주석처리 팝업에서 opener로 넘기는 값이었음 -->
...
function CheckForm() {
	...
	if(!document.form1.sel_paymethod.length) {//라디오 박스가 1개라면
