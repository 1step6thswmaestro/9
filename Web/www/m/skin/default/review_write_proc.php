<?
	...
	if(strlen($mode) <= 0 || $mode != "write"){
		echo '<script>alert("정상적인 방법이 아니므로 접근할 수 없습니다.");location.replace("/m/productdetail_tab03.php?productcode='.$productcode.'");</script>';exit;
	...
	echo '<script>alert("'.$returnmsg.'");location.replace("/m/productdetail_tab03.php?productcode='.$productcode.');</script>';
	exit;
?>