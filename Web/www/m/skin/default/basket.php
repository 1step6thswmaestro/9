	...
	while($vgrp=mysql_fetch_object($res)) {
		...
		while($row = mysql_fetch_object($result)) {
			...
			if (strlen($row->option_price)>0 && $row->opt1_idx==0) {
				...
				echo "<script>alert('필수 선택 옵션 항목이 있습니다.\\n옵션을 선택하신후 장바구니에\\n담으시기 바랍니다.');location.href=\"".$Dir."m/productdetail.php?productcode=".$row->productcode."\";</script>";
				exit;
			}
...
?>
	...
	<table class="" align="center" border="0" width="100%">
		<tr>					
			<td align="center" style="padding:5px">쇼핑하신 상품이 없습니다.</td>
		</tr>
	</table>
		<?
	} else {
...
?>
			...
			<section class="basic_btn_area btn_w1 btn_fs1">
				<div style="margin:10px 0px;">
					<a href="#" class="button white bigrounded" onClick="javascript:basket_clear()">장바구니비우기</a>
					<a href="#" class="button white bigrounded" onClick="location.href='<?=$shopping_url?>'">계속쇼핑</a>
					<a href="#" class="button blue bigrounded" onClick="location.href='login.php?chUrl=<?=urlencode("order.php")?>'">주문하기</a>
				</div>
			</section>
			<!-- //버튼 -->
<?
