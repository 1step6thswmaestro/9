...
<div id="content">
	...
	<div id="board_view">
		...
		<div id="contents_area"><?=$view_content?></div>
	</div>
	<div class="qna_view_bt">
		<a class="button black bigrounded" href="board_list.php?board=<?=$view_board?>" rel="external">목록보기</a>
...
//_pr($com_list);
	...
	for ($jjj=0;$jjj<count($com_list);$jjj++) {
		...
		if($adminCommNums > 0) {
			$adminComment .= "<div style=\"background:#f9f9f9; color:#888888; border:1px solid #f5f5f5; margin-top:5px; padding:5px 10px 3px 10px;\">";
			while( $adminCommRow = mysql_fetch_assoc ( $adminCommResult ) ) {
				$adminComment .= "<img src=\"".$imgdir."/icon_reply.gif\" alt=\"\" /> <strong>관리자</strong> : ".$adminCommRow['comment']; //(".$adminCommRow['reg_date'].")
			}
			$adminComment .= "</div>";
		}
