...
<div class="orderdetailwrap">
			...
			<div class="orderdetail_pr_list">
				<table cellpadding="0" cellspacing="0" border="0" width="100%" class="orderdetail_pr_table">
					<tbody>
						...
						<tr>
							<th>배달주소</th>
							<td><?=ereg_replace("주소 :","<br>주소 :",$_ord->receiver_addr)?></td>
						</tr>
						<tr>
							<th>결제방법</th>
							<td>
						...
						<tr>
							<th>결제금액</th>
							<td><font color=red><b><?=number_format($_ord->price)."원</b></font>".($_ord->reserve>0?"(적립금 ".number_format($_ord->reserve)."원 공제)":"")?></td>
						</tr>
						<?
	...
	<div style="text-align:center; margin:15px 0px;">
		<A HREF="javascript:window.close();" class="button black small">X 창닫기</A>
		<?
		if($print!="OK") {
			if (
			...
			|| ($_data->ordercancel==1 && preg_match("/^(B){1}/", $_ord->paymethod) && strlen($_ord->bank_date)<12 && $_ord->deli_gbn=="N" && getDeligbn("N",true)) /*주문결제완료 전에만 취소가 가능하며 무통장입금으로 입금전 미처리된 주문서일 경우 가능*/
				...
				if(!preg_match("/^(Q){1}/", $_ord->paymethod)) {
					echo "<a href=\"javascript:order_cancel('".$_ord->tempkey."', '".$_ord->ordercode."','".$_ord->bank_date."')\" onMouseOver=\"window.status='주문취소';return true;\" class=\"button white small\">전체주문취소</a>\n";
				}
			...
			&& strlen($_ShopInfo->getMemid())>0 /* 비회원은 내용삭제안되게 */) {
				echo "<a href=\"javascript:order_del('".$_ord->tempkey."', '".$_ord->ordercode."')\" onMouseOver=\"window.status='내용삭제';return true;\" class=\"button white small\">내용삭제</a>\n";
			}
