...
<?
		if(strlen($leftbody)>0) {
			...
			if (strlen($design_type)>0 && $design_type!="MAI"){
				$boardsep=$design_type;
				switch($design_type){
					case "BOU": // 고객센터 list_customer
						$boardlistSQL = "SELECT board_name, board FROM tblboardadmin WHERE list_customer = 'Y' AND use_hidden !='Y' ";
					break;
					case "BOM": //커뮤니티 list_community
						$boardlistSQL = "SELECT board_name, board FROM tblboardadmin WHERE list_community = 'Y' AND use_hidden !='Y'";
					break;
					case "BTC": // 기타 list_etc
						$boardlistSQL = "SELECT board_name, board FROM tblboardadmin WHERE list_etc = 'Y' AND use_hidden !='Y'";
					break;
				}
				if(strlen($boardlistSQL)>0){
					if(false !== $boardlistRes = mysql_query($boardlistSQL,get_db_conn())){
						$boardlistcount = mysql_num_rows($boardlistRes);
						if($boardlistcount>0){
							$boardlists ='<ul>';
							while($boardlistRow = mysql_fetch_assoc($boardlistRes)){
								$boardlists .='<li><a href="'.$Dir.'board/board.php?board='.$boardlistRow['board'].'&boardsep='.$newobj->menu_code.'">'.$boardlistRow['board_name'].'</a></li>';
								//$boardlists .=$boardlistRow['board_name'];
							}
							$boardlists .='</ul>';
						}
						mysql_free_result($boardlistRes);
					}
				}

				array_push($pattern,"(\[BOARD_LEFT_LIST\])");
				array_push($replace,$boardlists);
			}
