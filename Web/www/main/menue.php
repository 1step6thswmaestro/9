...
if($row->left_set=="Y") {
	...
	switch(substr(strrchr(getenv("SCRIPT_NAME"),"/"),1)) {
		case "board.php":
			$boardlisttitleimg="main_skin3_com_title.gif";
			if(!_empty($_REQUEST['boardsep'])){
				$design_type=trim($_REQUEST['boardsep']);
			}else{
				$boardsepSQL="SELECT list_customer, list_community, list_etc FROM tblboardadmin WHERE board ='".$_REQUEST['board']."' LIMIT 0,1";
				if(false !== $boardsepRes = mysql_query($boardsepSQL,get_db_conn())){
					$boardsepcount = mysql_num_rows($boardsepRes);
					if($boardsepcount>0){
						$boardsep_cou = mysql_result($boardsepRes,0,0);
						$boardsep_com = mysql_result($boardsepRes,0,1);
						$boardsep_etc = mysql_result($boardsepRes,0,2);
					}
					@mysql_free_result($boardsepRes);
				}
				if($boardsep_cou == "Y"){
					$design_type = "BOU";
				}else if($boardsep_com == "Y"){
					$design_type = "BOM";
				}else if($boardsep_etc == "Y"){
					$design_type = "BTC";
				}else{
					$design_type = "BTC";
				}
			...
			switch($design_type){
				case "BOU":
					$boardtargetlistSQL ="SELECT board_name, board FROM tblboardadmin WHERE list_customer = 'Y' AND use_hidden !='Y' ";
				break;
				case "BOM":
					$boardtargetlistSQL ="SELECT board_name, board FROM tblboardadmin WHERE list_community = 'Y' AND use_hidden !='Y' ";
				break;
				case "BTC":
					$boardtargetlistSQL ="SELECT board_name, board FROM tblboardadmin WHERE list_etc = 'Y'AND use_hidden !='Y' ";
				break;
			}

			break;
		case "community.php":
			$pagetype =!_empty($_GET['code'])?trim($_GET['code']):"";
			$boardtargetlistSQL="";
			switch($pagetype){
				case "1": // 고객센터
					$boardtargetlistSQL ="SELECT board_name, board FROM tblboardadmin WHERE list_customer = 'Y' AND use_hidden !='Y'";
					$boardlisttitleimg = "main_skin3_help_title.gif";
				break;
				case "2": // 커뮤니티
					$boardtargetlistSQL ="SELECT board_name, board FROM tblboardadmin WHERE list_community = 'Y' AND use_hidden !='Y'";
				break;
		...
		default:
			$boardlisttitleimg="main_skin3_com_title.gif";
			$boardtargetlistSQL ="SELECT board_name, board FROM tblboardadmin WHERE use_hidden !='Y'";
		break;
	}

$boardtargetlistcount=0;
$boardname=$boardcodename =array();

if(strlen($boardtargetlistSQL)>0){
	
	if(false !==$boardtargetlistRes = mysql_query($boardtargetlistSQL,get_db_conn())){
		$boardtargetlistcount = mysql_num_rows($boardtargetlistRes);
		if($boardtargetlistcount>0){
			while($boardlistRow = mysql_fetch_assoc($boardtargetlistRes)){
				array_push($boardname,$boardlistRow['board_name']);
				array_push($boardcodename,$boardlistRow['board']);
			}
		}
		mysql_free_result($boardtargetlistRes);
	}
}
	...
	$boardloop=0;
	if(_array($boardname)){
		foreach($boardname as $brname){
			if(file_exists($imagepath."easy".$boardcodename[$boardloop].".gif")) {
				$arr_menu[3].= "<tr><td><A HREF=\"".$Dir.BoardDir."board.php?board=".$boardcodename[$boardloop]."\"><img src=\"".$imagepath."easy".$boardcodename[$boardloop].".gif\" border=0 align=absmiddle></A></td></tr>\n";
			} else {
				$arr_menu[3].= "<tr><td height=22 style=\"padding-left:5\"><A HREF=\"".$Dir.BoardDir."board.php?board="$boardcodename[$boardloop]."\"><FONT class=\"leftcommunity\">".$brname."</FONT></A></td></tr>\n";
			}
			$boardloop++;
		}
	}
