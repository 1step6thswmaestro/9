...
<table align="center" border=0 cellpadding=0 cellspacing=0 width="<?=$maxsize?>">
<tr>
	<td align=center>
	<table border=0 cellpadding=0 cellspacing=1 bgcolor=#DADADA>
<?
	for($i=0;$i<$y;$i++) {
		if($i==0) echo "<tr height=70 bgcolor=#FFFFFF>\n";
		if($i>0 && $i%5==0) echo "</tr><tr height=70 bgcolor=#FFFFFF>\n";
		...
		if($xsize[$i]>$ysize[$i]) echo " width=60";
		else echo " height=60";
		echo "></a></td>";
	}
	if($i%5!=0) {
		if($i>5) {
			for($j=($i%5);$j<5;$j++) {
				echo "<td width=70 align=center bgcolor=#ffffff></td>";
			}
		}
		echo "</tr>\n";
	}
