...
$paystate=$_POST["paystate"];  //결제상태
$deli_gbn=$_POST["deli_gbn"]; // 배송단계 , 발송준비 :S , 배송완료 : Y, 미처리 : N, 반송 : R, 취소요청 : D, 환불대기 : E, 배송(정산보류 ): H
$s_check=$_POST["s_check"];	//cd:주문코드 ,mn : 구매자성명, mi: 구매회원아이디,cn:비회원주문번호  <-- 검색조건


$search_contents=!_empty($_POST["search"])?trim($_POST["search"]):""; // 검색어
$s_date=!_empty($_POST["s_date"])?trim($_POST["s_date"]):""; //배송일자 :deli_date, 입금일자 : bank_date, 주문일자 : ordercode 처리기준
...
$excelval=array(
	...
	array("받는분성명"								,&$receiver_name),		#11
	array("전화번호 비상전화"						,&$receiver_tel),		#12
	array("받는분전화번호"						,&$receiver_tel1num),	#13
	array("받는분기타연락처"						,&$receiver_tel2num),	#14
	...
	array("받는분주소(전체 분할)"									,&$addr),				#19
	array("전달사항"								,&$message),			#20
	array("품목명"									,&$product),			#21
	...
	array("보내는분성명"								,&$sendname),		#48
	array("보내는분주소(전체 분할)"								,&$sendadress),		#49
	array("보내는분전화번호"								,&$sendnumber)		#50
);

$sendname=$sendadress=$sendnumber="";
...
if($isproduct=="Y" || $isproductall=="Y") {
	...
	if(($s_date=="bank_date" || $s_date=="deli_date") && strlen($ordercodes)==0) {
		$tablecode=$s_date;
		$tempordercode="";
	...
	if(strlen($ordercodes)>0){
		...
		if($tablecode == "bank_date"){
			//$sql.= "b.ordercode=a.ordercode WHERE a.".$tablecode." >= '".$search_s."' AND a.".$tablecode." <= '".$search_e."' ";
			$sql.= "b.ordercode=a.ordercode WHERE (a.".$tablecode." >= '".$search_s."' AND a.".$tablecode." <= '".$search_e."') OR (a.ordercode >= '".$search_s."' AND a.ordercode <= '".$search_e."') ";
		}else{
			$sql.= "b.ordercode=a.ordercode WHERE a.".$tablecode." >= '".$search_s."' AND a.".$tablecode." <= '".$search_e."' ";
		}
	...
	if(strlen($search_contents)>0){
		switch($s_check){
			case "cd": // 주문코드
				$sql.= "AND a.ordercode = '".$search_contents."' ";
			break;
			case "mn": // 구매자성명
				$sql.= "AND a.sender_name = '".$search_contents."' ";
			break;
			case "mi": // 구매회원 아이디
				$sql.= "AND a.id = '".$search_contents."' ";
			break;
			case "cn": // 비회원주문번호
				$sql.= "AND a.id LIKE 'X".$search_contents."%' ";
			break;
		}
	}
	...
	switch($s_date){
		case "deli_date":
			$sql.="AND (a.deli_date IS NOT NULL OR a.deli_date !='') ";
			$sql.="ORDER BY a.deli_date DESC ";
		break;
		default:
	...
	if(strlen($search_contents)>0){
		switch($s_check){
			case "cd": // 주문코드
				$sql.= "AND ordercode = '".$search_contents."' ";
			break;
			case "mn": // 구매자성명
				$sql.= "AND sender_name = '".$search_contents."' ";
			break;
			case "mi": // 구매회원 아이디
				$sql.= "AND id = '".$search_contents."' ";
			break;
			case "cn": // 비회원주문번호
				$sql.= "AND id LIKE 'X".$search_contents."%' ";
			break;
		}
	}
...
while ($row=mysql_fetch_object($result)) {
	if ($temp!=$row->ordercode) {
		...
		$deli_date="=\"".substr($row->deli_date,0,4)."-".substr($row->deli_date,4,2)."-".substr($row->deli_date,6,2)."\"";
