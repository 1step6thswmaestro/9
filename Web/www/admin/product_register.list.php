...
<?
			if(strlen($code)==12) {
				...
				if($row && ereg("X",$row->type)){
					...
					$sql0.= "FROM tblproduct AS a LEFT OUTER JOIN tblcategorycode AS b ON a.productcode=b.productcode WHERE 1=1 ";
					$sql0.= $qry;
					...
					$sql.= "a.buyprice,a.quantity,a.reserve,a.reservetype,a.addcode,a.display,a.vender,a.tinyimage,a.selfcode,a.assembleuse, a.reservation ";
					$sql.= "FROM tblproduct AS a LEFT OUTER JOIN tblcategorycode AS b ON a.productcode=b.productcode WHERE 1=1 ";
					$sql.= $qry." ";
					...
					while($row=mysql_fetch_object($result)) {
						...
						echo "		</div>\n";
						echo "	</td>\n";
						echo "	<TD class=\"td_con1\" align=\"left\" style=\"word-break:break-all;\">".$reservationView."<img src=\"images/producttype".($row->assembleuse=="Y"?"y":"n").".gif\" border=\"0\" align=\"absmiddle\" hspace=\"2\"><A HREF=\"javascript:ProductSend('modify','".$row->productcode."');\">".$row->productname.($row->selfcode?"-".$row->selfcode:"").($row->addcode?"-".$row->addcode:"")."</A>&nbsp;</td>\n";
