...
if ($mode=="copy" || $mode=="move") {
		...
		if (strlen($cproductcode[$i])==18) {
			...
			if ($row=mysql_fetch_object($result)) {
				...
				if (strlen($row->maximage)>0) {
					$maximage=$copycode.$newproductcode.strtolower(strrchr($row->maximage,'.'));	//2015-06-03 파일확장자명을 3글자만 가져오지않고 전체를 가져오도록 수정/ wideimage가 확장자명 이전에 .이 붙지않는 문제 수정
				...
				if (strlen($row->minimage)>0) {
					$minimage=$copycode.$newproductcode."2".strtolower(strrchr($row->minimage,'.'));
				...
				if (strlen($row->tinyimage)>0) {
					$tinyimage=$copycode.$newproductcode."3".strtolower(strrchr($row->tinyimage,'.'));
				...
				if (strlen($row->wideimage)>0) {
					$wideimage=$copycode.$newproductcode.strtolower(strrchr($row->wideimage,'.'));
				...
				$sql.= "regdate			= now(), ";
				$sql.= "modifydate		= now(), ";
				$sql.= "content			= '"._escape($content,false)."' ";
				if(false === $insert = mysql_query($sql,get_db_conn())){
					_alert('상품 이동/복사중 오류가 발생 했습니다. 개발 지원에 문의 하시기 바랍니다.','-1');
					exit;
				}
				...
				if ($mode=="move") {
					...
					$multiCateSQL = "
						...
						WHERE
							`categorycode`= '".substr($cproductcode[$i],0,12)."'
							AND
	...
	for($i=0;$i<$cnt;$i++){
		...
		$wideimage = $Dir.DataDir."shopimages/wideimage/".$prcode[$i]."*";
...
<tr>
	...
	<tr>
		<td width="100%" style="padding-top:2pt; padding-bottom:2pt;" height="30"><B><span class="font_orange">* 정렬방법 :</span></B> <A HREF="javascript:GoSort('date');">진열순</a> | <A HREF="javascript:GoSort('productname');">상품명순</a> | <A HREF="javascript:GoSort('price');">가격순</a>
		<div style="width:200px; float:right; text-align:right">출력갯수
			<select name="clist_num" onchange="javascript:document.form1.submit();">
				<?
					$perRowsArr = array('10','20','30','50','100','200');
