...
// 부분취소 체크
$pChk = "SELECT SUM(p.price) as rcSum FROM tblorderproduct p WHERE p.deli_gbn IN('Y','N','X','S') AND p.productcode = '99999999995X' AND p.ordercode=a.ordercode";
//2015-03-06 적립금 환불금 조회
$CRChk = "select cancel_reserve from part_cancel_reserve where ordercode = a.ordercode";

// 입금
$Yif = " (a.pay_admin_proc='Y' || a.pay_admin_proc='N') && ( a.deli_gbn='Y' || a.deli_gbn='S' || a.deli_gbn='X' || a.deli_gbn='D' || a.deli_gbn='E' || a.deli_gbn='H' ) || ( IF( (".$pChk.") > 0 , TRUE, FALSE)) "; //2015-03-06 주문취소 금액 반영
