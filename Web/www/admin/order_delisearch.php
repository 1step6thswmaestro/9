...
if($search_s) {
	if(substr($search_s,0,8)==substr($search_e,0,8)) {
		if($s_date =="bank_date"){
			$qry.= "WHERE (a.".$s_date." LIKE '".substr($search_s,0,8)."%') OR (a.ordercode >= '".substr($search_s,0,8)."%') ";
		}else{
			$qry.= "WHERE a.".$s_date." LIKE '".substr($search_s,0,8)."%' ";
		}
	} else {
		if($s_date =="bank_date"){
			$qry.= "WHERE (a.".$s_date." >= '".$search_s."' AND a.".$s_date." <= '".$search_e."') OR (a.ordercode >= '".$search_s."' AND a.ordercode <= '".$search_e."') ";
		}else{
...
function OrderExcel() {
	document.form1.action="order_delisearch_excel.php";
	document.form1.submit();
	document.form1.action="";
}
...
function OrderCheckExcel() {
	...
	document.checkexcelform.action="order_delisearch_excel.php";
